...
export type ErrProps = {
  ...
  permalink: PermalinkModel;
};

const Contact: any = ({ setOpen, permalink }: ErrProps): JSX.Element => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    axios({
      url: `${API_BASE}/courier?id=${permalink.id}`,
      method: 'PUT',
      data: {
        title,
        slug,
  ...
  const setDefault = () => {
    setTitle(permalink.title);
    setSlug(permalink.slug);
    setCode(permalink.value);
    setDesc(permalink.description);
  };
