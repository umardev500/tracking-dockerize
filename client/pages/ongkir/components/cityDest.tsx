...
const CityDest: any = (): JSX.Element => {
    ...
    if (cost.provinceDest.id) {
      ...
      axios({ url: `${API_BASE}/city?province=${cost.provinceDest.id}` })
        .then((res) => {
          ...
          // Set list of cities
          dispatch({ type: 'SET_CITY_DATA_DEST', payload: results });
          // If the province selected not null
  ...
  const handleClick = (item: any) => {
    dispatch({ type: 'SET_CITY_DEST', payload: { cityDest: { name: item.city_name, id: item.city_id } } });
    toggleMenu();
  };

  CityDest.handleClickOutside = (): void => {
    setOpen(false);
  };
  ...
  return (
    <>
      ...
      <div className="lg:ml-1 lg:mr-1 mb-4">
        <div className="relative">
          <button onClick={toggleButton} type="button" className="relative overflow-hidden flex items-center w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 h-10 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" aria-haspopup="listbox" data-toggle="dropdown" aria-expanded="true" aria-labelledby="listbox-label">
            <span className="flex items-center">
              <span className="block truncate font-roboto uppercase">{cost.cityDest.id ? cost.cityDest.name : 'KOTA TUJUAN'}</span>
            </span>
          ...
          {isOpen ? (
            <ul tabIndex={-1} className="absolute z-10 bg-white shadow-lg w-full mt-1 max-h-56 rounded-md py-1 text-base lg:text-sm ring-1 ring-black ring-opacity-5 overflow-auto">
              {costData.citiesDest.length === 0 || isProvChanging ? (
              ...
              {costData.citiesDest.length > 0 && isProvChanging !== true
                ? costData.citiesDest.map((item: any, index: any) => {
                    return (
...
const outsideClickConfig = {
  handleClickOutside: () => CityDest.handleClickOutside
};

export default onClickOutside(CityDest, outsideClickConfig);
